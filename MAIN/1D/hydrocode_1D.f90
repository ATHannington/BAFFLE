...
!********************************************************************************
!* @file hydrocode_1D.f90
!*
!* @MAIN Program file for 1D BAFFLE: Fortran 90 version
!*
...
!	Set surface areas of faces
	xArea=1.d0!((zrangeMax-zrangeMin)/NcZr)*((yrangeMax-yrangeMin)/NcYr)
	yArea=1.d0!((xrangeMax-xrangeMin)/NcYr)*((zrangeMax-zrangeMin)/NcZr)
...
!	Assign starting values from init_conds, and mid-points to cells array:
	Do while (k<=1)!NcZ)
		...
		Do while (j<=1)!NcY)
			...
			Midy=0.d0!(dely/(NcY))*(-0.5d0)
			Midz=0.d0!(delz/(NcZ))*((k-1)-0.5d0)!Sets the middle of the cell to be at the middle of the kth fraction of the z
				else if (k==1) then
					Midx=(delx/(NcX))*((i-1)-0.5d0)!Sets the middle of the cell to be at the middle of the ith fraction of the x rang
			Midy=0.d0!(dely/(NcY))*((j-1)-0.5d0)!Sets the middle of the cell to be at the middle of the jth fraction of the y rangee
			Midz=0.d0!(delz/(NcZ))*(-0.5d0)

				else
					Midx=(delx/(NcX))*((i-1)-0.5d0)!Sets the middle of the cell to be at the middle of the ith fraction of the x range
			Midy=0.d0!(dely/(NcY))*((j-1)-0.5d0)!Sets the middle of the cell to be at the middle of the jth fraction of the y range
...
!			outside of the real cell range:			
				If ((i==1).or.(i==NcX)) then!.or.(j==1).or.(j==NcY).or.(k==1).or.(k==NcZ)) then
...
!	Calculate dimensions of each cell:
	...
	delmin=delx!minval((/delx,dely,delz/))
...
!	For first cell only, set new minimum timestep:
	If ((i==1))then!.and.(j==1).and.(k==1)) then
